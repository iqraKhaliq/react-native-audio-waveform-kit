...
export const useWaveformBars = (uri, providedBars) => {
  const [bars, setBars] = useState(providedBars && providedBars.length > 0 ? providedBars : null);
  const [loading, setLoading] = useState(!(providedBars && providedBars.length > 0));
  useEffect(() => {
