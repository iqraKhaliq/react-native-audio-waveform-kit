...
const PlayerWaveform = /*#__PURE__*/forwardRef(({
  ...
  const bgColor = stateTheme.backgroundColor ?? (sent ? colors.lightGreen : colors.lightGrey);
  const waveBg = seekColor ?? stateTheme.waveformBg ?? (sent ? colors.rgba002 : colors.rgba015);
  const waveFg = filledSeekColor ?? stateTheme.waveformFg ?? (sent ? colors.darkGreen : colors.black);
  const dotColor = seekDotColor ?? stateTheme.scrubberDot ?? colors.blue;
  const timeColor = stateTheme.timerText ?? (sent ? colors.darkGreen : colors.grey);
  ...
  const gesture = Gesture.Pan().runOnJS(true).onStart(() => {
